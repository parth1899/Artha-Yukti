...
def process_query_concurrent():
    ...
    # Call helper functions concurrently.
    with ThreadPoolExecutor(max_workers=3) as executor:
        future_to_component = {
            executor.submit(fetch_news, session.extracted.news): "news",
            # executor.submit(fetch_stock, session.extracted.stock): "stock",
            # executor.submit(fetch_twitter, session.extracted.twitter): "twitter"
        }
        results = {}
